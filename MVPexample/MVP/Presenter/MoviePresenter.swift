...
//  mansTV
//
//  Created by Matiss Mamedovs on 11/05/2018.
...
class MoviePresenter {
    ...
    fileprivate func setLabelsTitlesAndSizes() {
        ...
        if let seasons = movie?.seasons, !seasons.isEmpty {
            movieView?.setCurrentSeasonsLabel(season: Int(movie?.activeSeason))
        }
    ...
    fileprivate func getPosterImage() {
        movie?.GetLargeImageAsync({ (image: UIImage?, id: String)   in
            if let movie = self.movie, movie.id == id {
                if let image = image {
                    DispatchQueue.main.async {
                        self.movieView?.setPosterImageView(image: image)
                    }
                } else {
                    movie.GetPosterImageAsync({ (image: UIImage?, id: String)   in
                        if let movie = self.movie, movie.id == id {
                            if let image = image {
                                DispatchQueue.main.async {
    ...
    fileprivate func loadRecommendations() {
        ...
        if self.recomendedMovies.isEmpty {
            ...
            if let categoryId = categoryID {
                DataHelper.getRecomendedContentByCategory(movie.id, categoryId: categoryId, excludeSeriesID: movie.series_id, completeCallback: recommendedMoviesReturned, errorCallback: recomendedMoviesFailed)
            } else {
                DataHelper.getRecomendedContentByVods(movie.id, completeCallback: recommendedMoviesReturned, errorCallback: recomendedMoviesFailed)
                movieView?.finishLoadingRecommendations()
            }
        } else {
    ...
    fileprivate func episodesReturned(_ movies: [Movie]) {
    ...
    fileprivate func recommendedMoviesReturned(_ result: [ContentObject]) {
        if result.count == 0 {
    ...
    func playMovie()
    {
        ...
        if !movieView!.loadingStream {
            if !movie.isFree && !LoginHelper.isLoggedIn {
                ...
                movieView?.holderView.login(selected: self.movie, message: message)
            } else {
                movieView?.startLoadingStream()
                
                DataHelper.GetToken(for: movie, completeCallback: tokenGranted, errorCallback: tokenFailed, holder: (movieView?.holderView)!)
            }
        }
    }
    ...
    func resetCWT() {
        self.movie?.continue_watching_time = 0
        DataHelper.resetContinueWatching((self.movie?.id)!, type: (self.movie?.type)!)
    }
    ...
    @objc public func seasonSwitchTap(currentSeasonExpander: UIImageView?) {
        if let seasons = movie?.seasons, let currentSeasonExpander = currentSeasonExpander {
